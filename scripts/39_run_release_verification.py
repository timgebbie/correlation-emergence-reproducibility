...
def main() -> int:
    ...
    checks = [
        ...
        _check("V2R-09", "boundary representation documented", all(token in readme for token in ("translation", "mode", "-d(phi)/dx", "weak/local", "exp(-mu*y^2)")), "all correction terms present", all(token in readme for token in ("translation", "mode", "-d(phi)/dx", "weak/local", "exp(-mu*y^2)"))),
        ...
        _check("V2R-11", "Figure 11 mid-price ACF", "Log-mid increment autocorrelation" in dependence_script, "explicit panel", "Log-mid increment autocorrelation" in dependence_script),
        ...
        _check("V2R-27", "final release identity", [publication["tag"], publication["release_title"], publication["archive_name"]], "fixed v2.0.0 identity", publication["tag"] == "v2.0.0" and publication["release_title"].startswith("v2.0.0 — ") and publication["archive_name"] == "correlation-emergence-reproducibility-v2.0.0.zip"),
