...
\Require Sources $s_n^{i(j)}$, optional additive fields $\delta_n^{i(j)}$, and directed
rates $\kappa_{jk}\geq0$
\Ensure Next densities and boundaries, coupling/source fields, and diagnostics
\State Apply any declared market-order consumption map once to $\varphi_n^{(j)}$; retain the pre-event boundaries
\State Freeze the post-consumption $\varphi_{n,\mathrm{pre}}^{i(j)}\gets\varphi_n^{i(j)}$ and
...
For market-order runs, density is consumed before the completed update, with the pre-event boundary retained for the source displacement. The coupling gradient is evaluated on the post-consumption density; the removed quantity is not injected again as an additive source. The optional additive-field term excludes any market-order consumption already applied before the update. These ordering details determine the implementation used in Figures 9--14.
...
\Ensure Own/cross impact means, independent-group standard errors, and active fractions
\ForAll{paths $p$}
  ...
  \EndFor
\EndFor
\State Average signs, symmetry partners and clock replicates within each independent primitive group; retain directional book pairs where required and compute errors across groups
