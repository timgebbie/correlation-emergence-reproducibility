...
\date{Software release v2.0.0 --- 2026}

\begin{document}
\maketitle

\begin{abstract}
This supplement records the analytical and simulation companion to ``Correlation emergence and the Epps effect in two coupled limit order books'' (arXiv:2606.14182). It evaluates the paper's ordinary and fractional correlation build-up factors, reports their sensitivities and identifiability limits, diagnoses finite-grid boundary representation, and verifies a corrected uniform-operational simulation followed by explicit book-specific previous-refresh subordination. The retained numerical evidence includes clock-only and corrected coupling-only conformity, a combined no-refit holdout, single-trade and meta-order impact, and mid-price-increment and trade-sign autocorrelations. The computations are model-conditional reproducibility evidence, not empirical calibration or mathematical proof.
\end{abstract}
...
\subsection{Corrected source and boundary representation}

The corrected translated source uses
\begin{equation}
...
tested the translation-mode correction first as a component and then in a
...
\clearpage
\section{Corrected uniform-operational simulation}
...
For book \(j\), let \(U_{j,m}\) be its completed operational refresh indices
and define
\begin{equation}
N_j(t)=\max\{m: T_{j,m}\leq t\},
\qquad
P_j^{\rm cal}(t)=P_j^{\rm op}(U_{j,N_j(t)}).
...
\subsection{Final estimator-aware Epps comparison}
...
envelope. The estimator-aware curve is the exact finite-grid, finite-step
conditional moment evaluated at the operational indices selected by the same
realised clocks as the simulation estimator; it is not a fitted replacement.

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth]{figures/figure-07-final-estimator-aware-epps-v2.pdf}
\caption{\textbf{Final estimator-aware Epps integration.} Clock-only, corrected coupling-only and combined results share one linear 0--400 second and 0--1.1 normalized-covariance scale. The combined no-refit holdout is compared with the paper's leading-order product and the exact finite-grid, finite-step conditional moment on the same realised clocks. The estimator-aware combined RMSE is 0.039719, standardized RMSE is 0.455132, and pointwise normal-band coverage is complete. No parameter, normalization or time map is refitted.}
\label{fig:final-epps}
\end{figure}

\subsection{Corrected translation-mode component}

The corrected coupling is tested separately under identity clocks before it is
...
For a centered sequence \(X_n\), the registered sample autocorrelation at lag
\(k\) is
\begin{equation}
\widehat\rho_X(k)=
...
\clearpage
\begin{landscape}
\input{tables/table-01-parameter-timescale-identifiability-v1.tex}
\end{landscape}

\begin{landscape}
\input{tables/table-02-numerical-benchmarks-v1.tex}
\end{landscape}
